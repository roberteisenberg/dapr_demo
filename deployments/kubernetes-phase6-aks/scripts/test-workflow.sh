#!/bin/bash
# Test the Dapr Workflow saga pattern on AKS
...
echo "========================================="
echo "Testing Dapr Workflow Saga Pattern on AKS"
echo "========================================="
...
curl -s -X POST "$DAPR_API/catalog-service/method/products" \
  ...
  -d '{
    "id": "aks-workflow-laptop",
    "name": "AKS Workflow Test Laptop",
    "description": "For testing saga pattern on Azure",
    "price": 999.99,
    "stock": 10
  }' | jq .
echo ""
...
RESULT=$(curl -s -X POST "$DAPR_API/workflow-service/method/workflows/order" \
  -H 'Content-Type: application/json' \
  -d '{
    "orderId": "aks-saga-001",
    "productId": "aks-workflow-laptop",
    "quantity": 2,
    "customerName": "AKS Test User",
    "customerEmail": "akstest@example.com"
  }')
echo "$RESULT" | jq .
...
curl -s "$DAPR_API/catalog-service/method/products/aks-workflow-laptop" | jq .
echo ""
...
curl -s -X POST "$DAPR_API/catalog-service/method/products" \
  -H 'Content-Type: application/json' \
  -d '{
    "id": "aks-expensive-item",
    "name": "AKS Expensive Item",
...
FAIL_RESULT=$(curl -s -X POST "$DAPR_API/workflow-service/method/workflows/order" \
  -H 'Content-Type: application/json' \
  -d '{
    "orderId": "aks-saga-fail-001",
    "productId": "aks-expensive-item",
    "quantity": 1,
    "customerName": "AKS Test User",
    "customerEmail": "akstest@example.com"
  }')
...
curl -s "$DAPR_API/catalog-service/method/products/aks-expensive-item" | jq .
echo ""
